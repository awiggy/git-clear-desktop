# Git Agent Clear 真实凭据实机验收清单

日期：2026-09-06（创建）。目标：把审计中标注"环境依赖"的功能升级为实测结论。

本地构建应用：项目下的 `dist/Git Agent Clear.app`；也可安装本仓库 Releases 中的正式版本。
...
以下是此前记录的环境状态，不代表每次发布时仍成立。临时目录、登录状态和密钥必须在实机验收前重新检查；发布流水线成功不会自动将下面的待验收项标记为完成。
...
| 隔离仓库与 Clear 应用 | 历史测试时在位 | 验收前重新检查；临时仓库可能已清理 |
...
- [ ] E1. 使用启用了更新源的旧版正式安装包点“检查更新”，应发现更高版本并只下载 `GitAgent-Clear-*` 资产；默认本地开发构建未启用自更新，不适用此项。
